...
public class HomeController {
	...
	/**
	 * Checks if the stream is available by querying the stream status URL. The
	...
	private boolean isStreamAvailable() {
		return ((Boolean) this.webClient.get().uri(this.streamStatusUrl, new Object[0]).retrieve().bodyToMono(Map.class)
				.map(response -> (Boolean) response.get("stream-available")).block()).booleanValue();
	}

	/**
	 * Handles GET requests to the root URL ("/") and populates the model with the
	 * current stream URL, temperature, humidity, and last update time.
	 *
	...
	public String index(Model model) {
		...
		if (sensorResponse == null) {
			...
			model.addAttribute("currentTemperature",
					String.format("%.1f", new Object[] { Double.valueOf(sensorResponse.getTemperature()) }));
			model.addAttribute("currentHumidity",
					String.format("%.1f", new Object[] { Double.valueOf(sensorResponse.getHumidity()) }));
