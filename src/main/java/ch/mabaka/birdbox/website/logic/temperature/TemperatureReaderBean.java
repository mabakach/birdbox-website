...
/**
 * A Spring component that periodically reads temperature and humidity data from
 * a sensor, updates the current temperature bean, saves the data to the
 * repository, and sends updates via WebSocket.
 */
@Component
public class TemperatureReaderBean {
	private static final Logger LOGGER = LoggerFactory
			.getLogger(ch.mabaka.birdbox.website.logic.temperature.TemperatureReaderBean.class);
	 ...
	 * populating the current temperature bean with the latest measurement from the
	 * repository.
	 */
	@PostConstruct
	public void init() {
		LOGGER.info("TemperatureReaderBean initialized, sensor URL: {}, timeout: {}ms", this.sensorUrl,
				Integer.valueOf(this.sensorTimeout));
		...
		repository.findFirstByOrderByMeasurementTimestampDesc().ifPresent(entity -> {
			currentTemperatureBean
					.setLatestSensorResponse(SensorResponseToTemperatureMessurementEntityConverter.convert(entity));
		});
	}

	/**
	 * Scheduled method that reads temperature and humidity data from the sensor
	 * every minute, updates the current temperature bean, saves the data to the
	 * repository, and sends updates via WebSocket.
	 */
	@Scheduled(fixedRate = 60000L)
	public void readTemperature() {
		try {
			SensorResponse response = (SensorResponse) this.restTemplate.getForObject(this.sensorUrl,
					SensorResponse.class, new Object[0]);
			if (response != null) {
				...
				TemperatureMeassurementEntity entity = SensorResponseToTemperatureMessurementEntityConverter
						.convert(response);
			...
			if (e.getCause() instanceof java.net.SocketTimeoutException) {
				LOGGER.error("Timeout while reading temperature from sensor (>{}ms)",
						Integer.valueOf(this.sensorTimeout));
			} else {
